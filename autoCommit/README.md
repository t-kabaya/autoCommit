...
- Download Qwen2.5-Coder-3B-Instruct GGUF model (~2GB)
...
The default Qwen2.5-Coder-3B model is optimized for speed. If it's still slow:

1. Check CPU usage - llama.cpp should use multiple cores
2. Consider using a smaller quantized model (e.g., Q3 instead of Q4)
...
Default model: **Qwen2.5-Coder-3B-Instruct** (Q4_K_M quantization)

- Size: ~2GB
- Context: 32K tokens
- Speed: Fast on modern CPUs
- Quality: Excellent for code and commit messages
...
Recommended models:
- Qwen2.5-Coder series (1.5B, 3B, 7B)
